...
gappredict_data = []
gappredict_merged_data = []
for gap_type in gap_types:
    is_fixed = 1 if gap_type == "fixed" else 0
    gaps_path = base_path + gap_type + "/"
    gaps = os.listdir(gaps_path)
    for gap in gaps:
        gappredict_fill_file = gaps_path + gap + "/align.exn"
        gappredict_fill = parse_exonerate_output(gappredict_fill_file)
        gappredict_fill_row = to_row(gappredict_fill, gap, is_fixed)
        gappredict_data.append(gappadder_fill_row)

save_csv(gappadder_data, out_path + "/gappadder.csv")
