...
class SlidingWindowExtractor:
    ...
    def _extract_all_kmers_from_sequence(self, sequences):
        input_kmers = []
        output_kmers = []
    ...
    def _extract_unique_kmers_from_sequence(self, sequences):
        input_kmers = []
        output_kmers = []
        read_set = set()
        for sequence in sequences:
    ...
    def extract_kmers_from_sequence(self, sequences, unique=False):
        if unique:
            return self._extract_unique_kmers_from_sequence(sequences)
        else:
            return self._extract_all_kmers_from_sequence(sequences)