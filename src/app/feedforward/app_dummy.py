...
def main():
    ...
    input_seq, output_seq, shifted_output_seq, input_stats_map = \
        helper.label_integer_encode_kmers(input_kmers, output_kmers, with_shifted_output=False)

    input_one_hot_cube = helper.encode(input_length, input_seq, as_matrix=as_matrix)
    output_one_hot_cube = helper.encode(bases_to_predict, output_seq, as_matrix=as_matrix)
